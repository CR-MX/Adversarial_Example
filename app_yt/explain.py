...
def compute_integrated_gradients(inp, baseline, net, target, n_steps=100):
    ...
    baseline : torch.Tensor
        Baseline image of the same shape as the 'inp'.
    ...
    """
    # esta ruta va desde la linea base hasta la imagen de entrada.
    # el último elemento de esta lista sera la imagen de entrada
    path = [baseline + a * (inp - baseline) for a in np.linspace(0, 1, n_steps)]
    # aquí calculamos el gradiente con respecto a todos los tensores
    grads = [compute_gradient(func, x, net=net, target=target) for x in path]
    # esta linea se aproxima en la integral
    ig = (inp - baseline) * torch.cat(grads[:-1]).mean(dim=0, keepdim=True)

    # devolvemos los gradiente integrados y el gradiente con respecto a las entradas.
    return ig, grads[-1]
if __name__ == "__main__":
    ...
    ig, inp_grad = compute_integrated_gradients(
        tensor, baseline, net, 291, n_steps
    )

    # tomo estos degradados escalados y multiplico la imagen de entrada de ellos
    ig_scaled = scale_grad(ig)
    inp_grad_scaled = scale_grad(inp_grad)

    _, (ax_baseline, ax_img, ax_inp_grad, ax_ig) = plt.subplots(1, 4)
    ...
    ax_inp_grad.set_title("Grdient Input")
